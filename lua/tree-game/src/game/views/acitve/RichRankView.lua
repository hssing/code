--
-- Author: Your Name
-- Date: 2015-11-10 22:54:27
--
local RichRankView = class("RichRankView", base.BaseView)


function RichRankView:init(  )
	...
	self.cloneItem:getChildByName("Image_4"):getChildByName("Text_1_10"):setString(res.str.RICH_RANK_DESC38)
...
function RichRankView:setData( data )

	--奖励回馈系数
	self.varData = conf.ActiveVar:getValueByName("quanmingtuhao")
...
--创建或刷新数据
function RichRankView:setItemsData(qmthList )
	---每个名次
	local k = 0
...
-----活动倒计时
function RichRankView:timeTick( )
...
function RichRankView:getTimeStr( leftTime )
	--self.leftTime = self.leftTime - 1
	local left = 0
...
--点赞成功
function RichRankView:praiseSucc( data )
	--self:setItemsData(data.qmthList)
	self.dzCount = data.todayZan
	self.leftPraiseCount:setString(string.format(res.str.RICH_RANK_DESC6, data.todayZan))
	local data1 = {}
...
--点赞
function RichRankView:praiseBtnCallback( send,etype )
	if etype == ccui.TouchEventType.ended then
		if self.dzCount <= 0 then
			G_TipsOfstr(res.str.RICH_RANK_DESC32)
			return
		end
		proxy.RichRank:reqPraise(send.index)
	end
end


--规则
function RichRankView:gzCallbacl( send,etype )
	-- body
	if etype == ccui.TouchEventType.ended then
...
--刷新
function RichRankView:refreshCallback( send,etype )
	-- body
	if etype == ccui.TouchEventType.ended then
...
--充值
function RichRankView:chargeCallback( send,etype )
	-- body
	if etype == ccui.TouchEventType.ended then
		G_GoReCharge()
	end
