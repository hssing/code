--[[
	ContestRewardView --奖励
]]

local ContestRewardView = class("ContestRewardView", base.BaseView)

function ContestRewardView:init()
	-- body
	self.ShowAll = true
	...
	self.btnguize =  self.view:getChildByName("Button_24_0")
	self.btnguize:addTouchEventListener(handler(self, self.onBtnGuize))

	local lab = self.view:getChildByName("Image_14"):getChildByName("Text_11")
	lab:setString(res.str.CONTEST_DEC30)


	--界面固定文本
	self.btnreward:setTitleText(res.str.CONTEST_TEXT10)
	self.btnguize:setTitleText(res.str.CONTEST_TEXT11)




	self:setBtnStatue(true)
	self:setData()
end

function ContestRewardView:onBtnGuize( sender_,eventtype )
	-- body
...
function ContestRewardView:setBtnStatue(status)
	-- body
	self.btnreward:setHighlighted(status)
	self.btnreward:setTouchEnabled(not status)

	self.btnguize:setHighlighted(not status)
	self.btnguize:setTouchEnabled( status)
end

function ContestRewardView:onOpenItem(sender_,eventtype )
	-- body
	if eventtype == ccui.TouchEventType.ended then
		--G_openItem(send:getTag())
		local mId = sender_:getTag()
...
function ContestRewardView:initItem(item,v)
	-- body
...
--专属称号
function ContestRewardView:initFirstItem(  )
	-- body
	...
	local str = sex..string.format("%02d",11)
	...
	--驯兽师之王
	local _img = display.newSprite(res.font.XUNSHOUSHIWANG)
	--local x = label:getPositionX() + label:getContentSize().width + 100
	_img:setAnchorPoint(cc.p(0.5,0.5))
	_img:setPositionX(posx+20)
	_img:setPositionY(posy + 20)
	_img:addTo(item)
	--[[local label2 = display.newTTFLabel({
	    text = res.str.CONTEST_DEC34,
	    font = res.ttf[1],
	    size = 30,
	    align = cc.TEXT_ALIGNMENT_LEFT,
	    color = COLOR[1], -- cc.c3b(R,G,B)
	})
	local x = label:getPositionX() + label:getContentSize().width + 50
	label2:setPositionX( x )
	label2:setPositionY(posy)
	label2:addTo(item)]]--

	self.listview:pushBackCustomItem(item)
end

function ContestRewardView:setData()
	-- body
	self.data = conf.Contest:getAllItemaward()
...
function ContestRewardView:onCloseSelfView()
	-- body
	self:closeSelfView()
end

return ContestRewardView